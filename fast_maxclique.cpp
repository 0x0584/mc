...
enumerator::enumerator(graph &&g) : in(g.in) {
  ...
  container_reserve_memory(neighs_degree, count());
  std::for_each(std::execution::par_unseq, mc_const_range(g.adj_lst),
                [&neighs_degree, &g](const graph::adjacency_t::value_type &e) {
                  neighs_degree[e.first] = std::accumulate(
                      mc_const_range(e.second), 0ul,
                      [&g](std::size_t total_degrees, const graph::vertex u) {
                        return g.neighbours(u).size() + total_degrees;
                      });
                });

  // switch the vertex container from std::unordered_map to std::vector
  adj_lst_orig.resize(count());
  std::transform(std::execution::par_unseq, mc_range(g.adj_lst),
                 adj_lst_orig.begin(), [](graph::adjacency_t::value_type &e) {
  ...
  std::sort(
      std::execution::par_unseq, mc_range(adj_lst_orig),
      [&](const adjacency_t::value_type &v, const adjacency_t::value_type &u) {
  ...
  container_reserve_memory(mapping, count());
  adj_mat.resize(count());
  std::for_each(std::execution::par_unseq, mc_range(adj_mat),
                [this](std::vector<bool> &mtx) { mtx.resize(count()); });
  adj_lst.resize(count());
  for (key v = 0; v < count(); ++v) {
    const adjacency_t::value_type &v_pair =
  ...
  for (key v = 0; v < count(); ++v) {
    const graph::neighbours_t &neighs = adj_lst_orig[v].second;
...
fast_maxclique::find_maxclique(algorithm_type algo, mc::size_t lower_bound,
  ...
  } else if (assert(enumtor.count()); upper_bound == 1) {
    max_clique.emplace_back(0);
  }

  // reverse the keys back into vertices, as they were enumerated beforehand
  std::vector<graph::vertex> clique = enumtor.unfold_keys(max_clique);

  if (not enumtor.is_clique(max_clique)) {
    throw std::runtime_error("NOT even a clique!!\n");
  }

  if (algo != algorithm_type::heuristic) {
    if (max_clique.size() < lower_bound) {
      ...
      throw std::runtime_error(err.c_str());
               ...
               not enumtor.in.is_maximal_size(max_clique.size())) {
      const std::string err =
          " Max Clique with size=" + std::to_string(max_clique.size()) +
          " is NOT maximal expected size=" +
          std::to_string(enumtor.in.max_clique_size()) + "\n";
      throw std::runtime_error(err.c_str());
    }
  }
...
void fast_maxclique::solution(algorithm_type algo, mc::size_t upper_bound) {
  auto begin = std::chrono::high_resolution_clock::now();

  if (algo == algorithm_type::hybrid) {
    ...
    if (solution(algorithm_type::heuristic, upper_bound); upper_bound_reached) {
      auto end = std::chrono::high_resolution_clock::now();
      log::info(algorithm_type::hybrid, "finished using",
                algorithm_type::heuristic, "only (in",
                log::time_diff(begin, end, log::bold), ")");
      return;
    }
  }
  ...
  std::vector<enumerator::key> W(enumtor.count());
  ...
  container_reserve_memory(pruned, enumtor.count());
  //
  ...
  while (not W.empty() && not abort_search && not upper_bound_reached) {
    ...
    const std::size_t percent = enumtor.count() - W.size();

#ifdef LOG
    log::info("waiting...", log::progress(percent, enumtor.count()), "in",
              log::time_diff(begin, percent_end, log::bold));
#else // minimal console logging
    if (log::duration(percent_begin, percent_end) >= 1.) {
      log::info(log::progress(percent, enumtor.count()), "in",
                log::time_diff(begin, percent_end));
    }
#endif
    ...
    if (old_max_clique_size != current_max_clique_size) {
      log::info("found max clique of", current_max_clique_size, "vertices");
      old_max_clique_size = current_max_clique_size;
    }

    std::vector<enumerator::colour> colours = enumtor.greedy_colour_sort(W);
    for (std::uint32_t thread_id = 0;
...
#ifdef LOG
        log::print(enumtor.vertex(v),
                   "has insufficient colours. abort search!");
#else // minimal console logging
        log::print("abort search!");
#endif
        abort_search = true;
        break;
      }
      ...
      threads[thread_id] = std::thread(
          [&, this](const std::uint32_t thread_id, const enumerator::key v,
            ...
            std::vector<enumerator::key> neighs =
                enumtor.neighbours(v, [&pruned](const enumerator::key u) {
            ...
            if (neighs.empty()) {
#ifdef LOG
              log::print(enumtor.vertex(v), "has no neighbours. abort search!");
#endif
              abort_search = true;
              return;
            }
            ...
            std::vector<enumerator::colour> colours =
                enumtor.greedy_colour_sort(neighs);
...
#ifdef LOG
            log::print_thread(thread_id, "branching on", enumtor.vertex(v),
...
#ifdef LOG
              std::ostringstream oss;
              oss << "found clique for " << enumtor.vertex(v) << " of "
                  << clique.size() << " vertices { ";
              for (const enumerator::key u : clique) {
                oss << enumtor.vertex(u) << " ";
              }
              oss << "}";
...
#ifdef LOG
            auto end = std::chrono::high_resolution_clock::now();
            log::print_thread(thread_id, "done with", enumtor.vertex(v),
      ...
      reading_is_complete.wait(reading_lock,
                               [&reading]() { return not reading; });
    }
  }
...
bool fast_maxclique::enlarge_clique_size(const std::uint32_t thread_id,
                                         mc::size_t &max_clique_size,
...
void fast_maxclique::branch_exact(const std::uint32_t thread_id,
  ...
  while (not neighs.empty() && not upper_bound_reached &&
    ...
    std::vector<enumerator::key> new_neighs = enumtor.neighbourhood(u, neighs);
    if (new_neighs.empty() || next_depth == upper_bound) {
      ...
      std::vector<enumerator::colour> new_colours =
          enumtor.greedy_colour_sort(new_neighs);
...
void fast_maxclique::branch_heuristic(
...
int main(int ac, const char *av[]) {
  ...
  try {
    mc::input in(std::cin);
    mc::enumerator enumtor(in);
    mc::fast_maxclique fmc = enumtor;

    std::size_t num_turns = ac != 1 ? std::size_t(std::atol(av[1])) : -1u;
    while (num_turns--) {
      fmc.find_maxclique(mc::fast_maxclique::algorithm_type::hybrid);
      fmc.find_maxclique(mc::fast_maxclique::algorithm_type::exact);
    }
