...
def extract_chunks_and_edges_per_chunk(text):
    ...
    if m:
        cv, cu = m.groups()[0:2]
        ev, eu = m.groups()[2:4]
        return float(convert_scaled(cv, cu, count_units)), float(
            convert_scaled(ev, eu, count_units)
        )
    return 0, 0
...
for i, graph in enumerate(graphs):
    ...
    proc = subprocess.run(
        [
            "osascript",
            "-e",
            f'display notification "[{i+1:2}/{len(graphs):2}] {graph} done in {graph_elapsed:.2f}s" with title "Benchmarking"',
        ]
    )
    if best_run:
