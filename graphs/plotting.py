...
phase_columns = ['ReadThr(MB/s)', 'ComputeDegreesThr(M/s)', 'ParseThr(M/s)', 'ConstructThr(M/s)']
...
for col in phase_columns:
    ax.plot(labels, df[col], marker='o', label=col)

for i, r in enumerate(ratio):
    if r < 0.5:
        ax.plot(labels[i], read_thr[i], marker='o', color='red', markersize=8)
...
if 'ReadTime(s)' in df:
    phase_times.append(df['ReadTime(s)']); phase_labels.append('Read')
if 'ParseTime(s)' in df:
    phase_times.append(df['ParseTime(s)']); phase_labels.append('Parse')
if 'ConstructTime(s)' in df:
    phase_times.append(df['ConstructTime(s)']); phase_labels.append('Construct')
if 'MergeVerticesTime(s)' in df:
    phase_times.append(df['MergeVerticesTime(s)']); phase_labels.append('MergeVertices')
if 'MergeEdgesTime(s)' in df:
    phase_times.append(df['MergeEdgesTime(s)']); phase_labels.append('MergeEdges')
if 'ComputeDegreesTime(s)' in df:
    phase_times.append(df['ComputeDegreesTime(s)']); phase_labels.append('ComputeDegrees')
...
ax.set_title('Normalized Benchmarking Times per Graph (Anomalies Outlined)')
...
ax.scatter(df_sorted['Edges'], df_sorted['TotalTime(s)'], c='darkblue', marker='o', label='Edges')
...
fig, ax = plt.subplots(figsize=(8, 6))
ax.scatter(df_sorted['StreamSize(B)'], df_sorted['EstimatedChunks'], c='steelblue', marker='o', label='Estimated Chunks')
ax.plot(df_sorted['StreamSize(B)'], df_sorted['EstimatedChunks'], c='steelblue', linewidth=1)
ax.scatter(df_sorted['StreamSize(B)'], df_sorted['EdgesPerChunk'], c='darkorange', marker='s', label='Edges per Chunk')
ax.plot(df_sorted['StreamSize(B)'], df_sorted['EdgesPerChunk'], c='darkorange', linewidth=1)
ax.set_xscale('log')
ax.set_yscale('log')
...
color_cpu = 'tab:red'
ax1.set_ylabel('Average CPU (%)', color=color_cpu)
cpu_bars = ax1.bar(labels, df['AvgCPU(%)'], color=color_cpu, alpha=0.6, label='Avg CPU (%)')
ax1.tick_params(axis='y', labelcolor=color_cpu)

color_mem = 'tab:blue'
ax2 = ax1.twinx()
ax2.set_ylabel('Peak Memory (MB)', color=color_mem)
mem_line, = ax2.plot(labels, peak_mem_mb, color=color_mem, marker='o', label='Peak Memory (MB)')
ax2.set_yscale('log')
ax2.tick_params(axis='y', labelcolor=color_mem)

plt.setp(ax1.get_xticklabels(), rotation=label_rot, ha='right')
fig.legend([cpu_bars, mem_line], ['Avg CPU (%)', 'Peak Memory (MB)'], loc='center left', bbox_to_anchor=(1.25, 0.5))
...
# Memory usage
for phase in ["Read", "Parse", "Construct"]:
    axes[1].plot(labels, df[f"PeakMem(B)/{phase}"] / (1024*1024), marker='o', label=phase)
...
def create_heatmap(metric, title, output_file, cm, compare_metric=None):
    pivot_table = (
        df.pivot_table(index='EdgeBin', columns='VertexBin', values=metric, aggfunc='mean', observed=False)
    ...
    for i in range(pivot_table.shape[0]):
        for j in range(pivot_table.shape[1]):
            ...
            if pd.notna(value):
                ax.text(j, i, f"{scale_int(value)}", ha='center', va='center', color='black', fontsize=15).set_path_effects([
    ...
    if compare_metric:
        comp_table = (
            df.pivot_table(index='EdgeBin', columns='VertexBin', values=compare_metric, aggfunc='mean', observed=False)
            .reindex(index=edge_labels, columns=vertex_labels)
        )
        ...
        for i in range(pivot_table.shape[0]):
            for j in range(pivot_table.shape[1]):
                if pd.notna(residuals.iloc[i, j]) and abs(residuals.iloc[i, j]) > threshold:
                    ax.add_patch(plt.Rectangle((j-0.5, i-0.5), .99, .99, fill=False, edgecolor='black', linewidth=1.5))
...
create_heatmap('ReadThr(MB/s)', 'Average Read Throughput (MB/s)', f'{output_dir}/heatmap_read_thr.png', plt.cm.RdYlGn)
create_heatmap('ReadTime(s)', 'Average Read Time (s)', f'{output_dir}/heatmap_read_time.png', plt.cm.RdYlGn_r, compare_metric='ParseTime(s)')    
create_heatmap('EdgesPerChunk', 'Average Edges Per Chunk', f'{output_dir}/heatmap_edges_per_chunk.png', plt.cm.RdYlGn)
